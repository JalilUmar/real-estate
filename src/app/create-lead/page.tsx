...
export default function CreateNewLead() {
    ...
    const handleCreateNewLead = async () => {
        await fetch("/api/users", {
            method: "POST",
            body: JSON.stringify({
                firstName, lastName, email, phone, address, title, password
            })
        })
        setFirstName("")
        setLastName("")
    ...
    return (
        <form method='POST' className='bg-blue-400 w-[500px] my-10 mx-auto py-5 grid justify-center gap-y-6 rounded-lg shadow-lg shadow-black'>
            <h2 className='text-center text-4xl font-sans font-bold pb-5'>Create new lead</h2>
            <input
                type="text"
                name="FirstName"
            ...
            <button onClick={handleCreateNewLead} className="bg-white mt-5 px-4 py-2 w-[350px] mx-auto text-xl font-sans font-semibold transition-all rounded-lg shadow-black shadow-md hover:scale-105 active:scale-100">Create Lead</button>
        </form>
    )
}
